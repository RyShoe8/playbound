...
After the live Morrowind room closed, the updated live agent passed the same OpenRA managed-room lifecycle: it started a room, reattached PID 797745 after an agent restart, and stopped it. A second short run appeared in the official OpenRA master as `147.93.133.235:1234`, with `players=0`, `mod=ra`, `version=release-20250330`. The `openra-master` player-query adapter can therefore be marked tested for the base OpenRA profile. No client joined, and no populated-load sample exists; `joinVerified`, profile `enabled`, and `rotationEligible` remain false. The 15-minute VPS timer is installed; its authenticated manual run returned `disabled`.
