/** Controls-only runner for the third explicitly approved catalog batch. */
...
const SLUGS = [
  "asphalt-legends", "star-wars-the-old-republic", "lord-of-the-rings-online", "dc-universe-online",
  "world-of-sea-battle", "star-trek-online", "guild-wars-2", "palia", "once-human", "where-winds-meet",
  "rainbow-six-siege", "enlisted", "albion-online", "holocure", "quake-champions", "counter-strike-2",
  "valorant", "league-of-legends", "dota-2", "genshin-impact", "team-fortress-2", "war-thunder",
  "starcraft", "star-wars-galaxies", "everquest", "tes-arena", "daggerfall", "gamebuddies-io",
  "pixreveal", "openarena", "freedoom",
] as const;
const SCRIPT_DIR = dirname(fileURLToPath(import.meta.url));
const PLATFORM_DIR = resolve(SCRIPT_DIR, "..");
const DATA_FILE = resolve(SCRIPT_DIR, "control-batches", "batch-3.json");
...
async function main() {
  ...
  console.log(`[controls batch 3] ${SLUGS.length} schema-valid games`);
  ...
  for (const slug of SLUGS) {
    ...
    if (!newDoc || withoutControls(oldDoc) !== withoutControls(newDoc)) throw new Error(`${slug}: a non-controls field changed`);
...
main().catch((error) => { console.error("fill-game-controls-batch-3 failed:", error instanceof Error ? error.message : error); process.exit(1); });
