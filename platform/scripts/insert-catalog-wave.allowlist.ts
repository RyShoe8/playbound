...
export const PATCH_GAME_FIELDS: Readonly<Record<string, readonly string[]>> = {
   ...
   * "Run the downloaded installer" on Windows regardless of the real
   * launcherInstall.kind, fixed 2026-09-25 — see the fix's own commit) found
   * live on these 8 games via a full sweep of every zip/jar-kind published
   * game's admin edit page: all eight ship a portable zip or (mindustry) a
   * .jar, never an installer, and the live text told players to run a
   * Windows-SmartScreen-triggering installer that doesn't exist. installSteps
   * regenerated by calling the now-fixed deriveInstallSteps() against each
