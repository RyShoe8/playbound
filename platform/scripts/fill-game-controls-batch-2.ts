/**
 * Add controls to the second explicitly approved catalog batch.
 *
 * SAFETY CONTRACT:
 * - The 25 allowed slugs are hardcoded; the script accepts no slug input.
 ...
 * - After writing, every document field other than controls is compared.
 */
...
const DATA_FILE = resolve(SCRIPT_DIR, "control-batches", "batch-2.json");
...
async function main() {
  ...
  console.log(`[controls batch 2] ${SLUGS.length} schema-valid games`);
...
main().catch((error) => {
  console.error("fill-game-controls-batch-2 failed:", error instanceof Error ? error.message : error);
  process.exit(1);
});
