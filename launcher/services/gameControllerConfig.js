...
function ysoccerHasDualSenseStickAxes(text) {
  ...
  return dualSenseEntries.some((entry) =>
    /xAxis:5,\s*yAxis:4,\s*button1:1,\s*button2:2(?:,|\})/.test(entry)
  );
}
  ...
  ysoccer: {
    ...
    apply(text, profile) {
      ...
      const configs = [
        ...
        `{class:JoystickConfig,name:DualSense Wireless Controller,xAxis:5,yAxis:4,button1:1,button2:2}`,
        `{class:JoystickConfig,name:Wireless Controller,xAxis:0,yAxis:1,button1:0,button2:1}`,
        `{class:JoystickConfig,name:PS5 Controller,xAxis:5,yAxis:4,button1:1,button2:2}`,
       ...
       * Xbox-compatible pads use the mappings below.
     *
     * Face buttons follow the same split. DirectInput enumerates a PlayStation
     * pad Square 0, Cross 1, Circle 2, Triangle 3, so 0/1 put confirm on Square
     * and back on Cross — playable, but not what the buttons say. 1/2 is Cross
     * to confirm and Circle to go back. Xbox pads keep 0/1, where A and B
     * already sit.
       *
      ...
      for (const name of joystickNamesFor(profile)) {
        if (!configs.some((c) => c.includes(`name:${name},`))) {
          ...
          const b1 = isDualSense ? 1 : 0;
          const b2 = isDualSense ? 2 : 1;
          configs.unshift(`{class:JoystickConfig,name:${name},xAxis:${xAxis},yAxis:${yAxis},button1:${b1},button2:${b2}}`);
        }
      }
