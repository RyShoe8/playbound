...
test("CLSID_DIRECTDRAW is the classic DirectDraw class", () => {
  ...
  assert.equal(CLSID_ALPHABLEND, "{B6803A0F-671C-4730-A802-BB0C2C4BDAC4}");
});

test("COM redirection covers DDrawCompat CLSIDs and maps FreeTrain CLSIDs to dx7vb/dx8vb/AlphaBlend.dll", () => {
  ...
  assert.ok(!DIRECTDRAW_COM_CLSIDS.includes(CLSID_ALPHABLEND), "{B6803A0F} must not be in DIRECTDRAW_COM_CLSIDS");
  ...
  assert.ok(MS_X86_DLLS.includes("dx8vb.dll"), "dx8vb.dll must be in MS_X86_DLLS");
  assert.ok(MS_X86_DLLS.includes("DirectDraw.AlphaBlend.dll"), "DirectDraw.AlphaBlend.dll must be in MS_X86_DLLS");
  const { DIRECTX8_COM_CLSIDS } = require("./directDrawWrapper");
  assert.ok(Array.isArray(DIRECTX8_COM_CLSIDS) && DIRECTX8_COM_CLSIDS.length >= 2);
  assert.ok(DIRECTX8_COM_CLSIDS.includes("{E7FF1300-96A5-11D3-AC85-00C04FC2C602}"));
});
...
test("ensureForGame copies from a local MS/x86 source without GitHub", async () => {
  ...
  for (const name of ["DDraw.dll", "D3DImm.dll", "D3D8.dll", "D3D9.dll", "dx7vb.dll", "dx8vb.dll", "DirectDraw.AlphaBlend.dll"]) {
