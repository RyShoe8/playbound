...
contextBridge.exposeInMainWorld("playbound", {
  ...
  getNetBirdStatus: () => ipcRenderer.invoke("get-netbird-status"),
  installNetBird: () => ipcRenderer.invoke("install-netbird"),
  getVigemStatus: () => ipcRenderer.invoke("get-vigem-status"),
  installVigem: () => ipcRenderer.invoke("install-vigem"),
  ...
  runTes3mpCommand: (partyId, command, targetPid) =>
    ipcRenderer.invoke("run-tes3mp-command", partyId, command, targetPid ?? null),
